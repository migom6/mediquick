...
class TestingDiagnosis(unittest.TestCase):
    ...
    def test_status(self):
        response = requests.get('http://'+env.HOST+':'+ env.PORT+'/diagnosis?symptoms=[233]&gender=male&year_of_birth=1988')
        print("Checking if status code is OK" )
        self.assertEqual(response.status_code, 200)

    #Checking whether given combinations of symptoms exists or not
    def test_combination(self):
        response = requests.get('http://'+env.HOST+':'+ env.PORT+'/diagnosis?symptoms=[233,144]&gender=male&year_of_birth=1988')
