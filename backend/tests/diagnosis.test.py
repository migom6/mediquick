...
class TestingDiagnosis(unittest.TestCase):
    ...
    def test_status(self):
        response = requests.get(
            f'http://{env.HOST}:{env.PORT}/api/diagnosis?symptoms=[233]&gender=male&year_of_birth=1988')
    ...
    def test_combination(self):
        response = requests.get(
            f'http://{env.HOST}:{env.PORT}/api/diagnosis?symptoms=[233,144]&gender=male&year_of_birth=1988')
